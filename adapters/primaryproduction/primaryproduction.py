...
def apply(env, params, l2product_files, date):
    ...
    zvals_fine = np.linspace(zvals.min, zvals.max, 100)  # Fine spaced depths for integration
...
def pp_trapezoidal_numerical_integration(zvals, qpar0, Cchl, KdMorel):
    if qpar0.shape == Cchl.shape and Cchl.shape == KdMorel.shape:
        ...
        for i in range(1, pp_tni.shape[0] - 1):
            for j in range(1, pp_tni.shape[1] - 1):
                if np.isnan(Cchl[i, j]) or not qpar0[i, j] or not KdMorel[i, j]:
                    continue
                pp_tni[i, j] = trapz(PP(zvals, qpar0[i, j], Cchl[i, j], KdMorel[i, j]), zvals)
    else:
...
def absorption(Cchl):
    ...
    [0.23925,0.25175,0.2665,0.27725,0.28625,0.29725,0.297,0.30275,0.30675,0.28,0.23575,0.19325,0.1535,0.123,0.104,0.099,0.1115,0.1205,0.1495,0.17375,0.188,0.16625,0.1715,0.185]])
